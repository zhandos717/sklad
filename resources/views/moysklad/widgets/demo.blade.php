    ...
    <script>
        ...
        window.addEventListener("message", function(event) {
            ...
            if (receivedMessage.name === 'Open') {
                ...
                oReq.open("GET", "<?=$getObjectUrl ?? null?>" + receivedMessage.objectId);
                oReq.send();
...
<body>
<p><b title="Информацию о текущем пользователе виджет может получить на своем бэкенде через Vendor API, используя contextKey">Текущий пользователь <span class="hint">(?)</span>:</b> <?=$uid ?? null?> (<?=$name ?? null?>)</p>
